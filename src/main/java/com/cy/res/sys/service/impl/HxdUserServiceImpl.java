...
public class HxdUserServiceImpl implements HxdUserService{
	...
	public int insertObject(HxdUser userData) {
		...
		//判断用户名是否已存在
		HxdUser loginnama = hxdUserDap.findOnebyLoginName(userData.getUserLoginName());
		if(loginnama!=null) 
		...
		int rows = hxdUserDap.insertObject(userData);
		if(rows<1)
	...
	public int isLogin(String username, String password) {
		...
		//查找用户名记录
		HxdUser user = hxdUserDap.findOne(username);
		...
		HxdUser userdata = hxdUserDap.findOneBySaltPassword(hex,id);
